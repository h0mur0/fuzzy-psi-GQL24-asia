...
void debugRecvResize(coproto::Socket* socket, T& data, const std::string& description = "") {
    ...
    namespace OT_for_FPSI{
        ...
        std::vector<std::vector<element>> run_ot_receiver_long_half_one(coproto::Socket& channel, BitVector& choices, const u64& numOTs, const u64 Msg_Length){
            ...
            {   
                ...
                recv.setBaseOts(baseSend);
                auto proto = recv.receive(choices, mask, prng, channel);
                auto half_result = macoro::sync_wait(macoro::when_all_ready(std::move(proto)));
        ...
        void run_ot_sender_long_half_one(coproto::Socket& channel, std::vector<std::array<std::vector<element>, 2>> sendMsg, const u64 Msg_Length)
        {
            ...
            if(numOTs <= OT_NUMS_BOUND)
            {
                ...
                for(u64 i = 0; i < numOTs; i++)
                {
                    ...
                    for(u64 j = 0; j < Msg_Length; j++){
                        // half_sendMsg_0[i * Msg_Length + j] = sendMsg[i][0][j] ^ prng0.get<block>();
                        half_sendMsg_1[i * Msg_Length + j] = sendMsg[i][1][j] ^ prng1.get<block>();
                    }
                }
        ...
        void last_ot_send(coproto::Socket* channel, std::vector<std::vector<u64>>* sender_elements, u64 dimension){
            ...
            OT_for_FPSI::run_ot_sender_long_half_one(*channel, send_results, dimension / 2);
    ...
    namespace fm_paillier{
        ...
        std::vector<block> bignumer_to_block_vector(const BigNumber& bn){
            ...
            for(auto i = 0; i < PAILLIER_CIPHER_SIZE_IN_BLOCK; i++){
                cipher_block[i] = block(((u64(ct[4 * i + 3]))<< 32) + (u64(ct[4 * i + 2])), ((u64(ct[4 * i + 1]))<< 32) + (u64(ct[4 * i])));
            }
            return cipher_block;
        }
        ...
        void block_to_prefix(const u32& x, const int& log_delta_pow_p, std::vector<block>& prefixes){
            block container;
    ...
    /////////////////////////////////////////////////////////
        void receiver_value_paillier_lp(const std::size_t& elements_size,
            ...
            for(u64 i = 0; i < elements_size ; i++){
                for(u64 j = 0; j < dimension; j++){
                    ...
                    //     // printf("\n");
                    //     ct_test[j] = ipcl::CipherText(paillier_key.pub_key, ct.getElement(0));
                    //     // std::cout << "values_bignumber_in_dimension 0 0" << ct.getElement(0) << std::endl;
                    // }
                    // // std::cout << "recv dimension" << j << ": " << ct.getElement(0) << std::endl;
        ...
        void sender_q_to_masked_distance_paillier_lp(const std::vector<std::vector<u64>>& elements, const std::vector<Rist25519_point>& vec_kq,
                                                    const std::vector<std::vector<block>>& codeWords, const ipcl::CipherText& vec_mask_ct,
            ...
            ipcl::initializeContext("QAT");
            std::vector<std::vector<BigNumber>> decode_result_in_dimension(dimension, std::vector<BigNumber>(elements.size()));
            ipcl::setHybridMode(ipcl::HybridMode::OPTIMAL);
            for(u64 i = 0; i < elements.size(); i++){

                for(u64 j = 0; j < dimension; j++){
                    std::vector<element> value = rb_okvs.decode(codeWords, get_key_from_k_d_x(vec_kq[i], j, elements[i][j]), PAILLIER_CIPHER_SIZE_IN_BLOCK);
            ...
            for(u64 j = 0; j < dimension; j++){
                masked_distances = masked_distances + ipcl::CipherText(pk, decode_result_in_dimension[j]);
        ...
        void sender_q_to_masked_distance_paillier_linfty(const std::vector<std::vector<u64>>& elements, const std::vector<Rist25519_point>& vec_kq,
                                                    const std::vector<std::vector<block>>& codeWords,
                                                    ipcl::CipherText& vec_masked_distance, std::vector<BigNumber>& additive_masks,
                                                    const u32& dimension, const i32& okvs_n, const ipcl::PublicKey& pk){
            ...
            ipcl::CipherText masked_distances(ipcl::CipherText(pk, decode_result_in_dimension[0]));
            ...
            vec_masked_distance = masked_distances + pk.encrypt(ipcl::PlainText(additive_masks));
            //vec_masked_distance = masked_distances;

            //std::cout << "[send]: send paillier cipher done" << vec_masked_distance.getElement(0) << std::endl;
        ...
        //             auto value = rb_okvs.decode(codeWords, get_key_from_k_d_x(vec_kq[i], j, elements[i][j]), PAILLIER_CIPHER_SIZE_IN_BLOCK);
        //             decode_result_in_dimension[j][i] = block_vector_to_bignumer(value);
        //         }
        //     }
        //     ipcl::CipherText masked_distances(pk.encrypt(ipcl::PlainText(vec_mask_u32)));
        //     for(auto j = 0; j < dimension; j++){
        //         masked_distances = masked_distances + ipcl::CipherText(pk, decode_result_in_dimension[j]);
        //     }
        //     for(auto i = 0; i < elements.size(); i++){
        ...
        void receiver_get_masked_distance_paillier_lp(const ipcl::CipherText& vec_masked_distance,
            ...
            ipcl::initializeContext("QAT");
            ipcl::setHybridMode(ipcl::HybridMode::OPTIMAL);
            ipcl::PlainText masked_distance_plain = paillier_key.priv_key.decrypt(vec_masked_distance);
        ...
        void receiver_get_result_paillier_linfty(const ipcl::CipherText& vec_masked_distance,
                                                    std::vector<BigNumber>& result_masks,
                                                    const ipcl::KeyPair& paillier_key){
            ...
            // std::cout << "[recv]: recv paillier cipher dec" << result_masks[0] % *(paillier_key.pub_key.getN()) << std::endl;
        ...
        void pad_send_prefixes_k(const std::vector<std::vector<DH25519_point>>& send_prefixes_k, std::vector<DH25519_point>& send_prefixes_k_net, const u64& max_prefix_num){
            ...
            for(auto iter : send_prefixes_k){
                send_prefixes_k_net.insert(send_prefixes_k_net.end(), iter.begin(), iter.end());
        ...
        void receiver_get_prefixes(const std::vector<u32>& masked_distance,
            ...
            for(auto iter : masked_distance){
                ...
                block_to_prefix(iter, log_delta_pow_p, temp_prefixes);
                //std::cout << "recv mask distance interval: " << iter - (u32)(pow(delta, p)) << ", " << iter + (u32)(pow(delta, p)) << std::endl;
                vec_prefixes.push_back(temp_prefixes);
            }
            return;
        }
        ...
        void prefixes_check(const std::vector<std::vector<DH25519_point>>& send_prefixes_kk, const std::vector<std::vector<DH25519_point>>& recv_prefixes_kk,
            ...
            for(auto i = 0; i < send_prefixes_kk.size(); i++){
                ...
                for(auto iter : send_prefixes_kk[i]){
                    auto it_find = find(recv_prefixes_kk[i].begin(), recv_prefixes_kk[i].end(), iter);
        ...
        void dh_check(const std::vector<DH25519_point>& send_kk, const std::vector<DH25519_point>& recv_kk,
                            BitVector& result){
        ...
        ipcl::KeyPair paillier_key, DH25519_number recv_dh_k, const std::string& result_file
            ...
            receiver_w_to_key_paillier_lp(*receiver_elements, *recv_vec_dhkk_seedsum, fmat_keys, dimension, delta);

            RBOKVS rb_okvs;
            rb_okvs.init(fmat_keys.size(), 0.1, lambda, seed);
            std::vector<std::vector<block>> codeWords_fmat(rb_okvs.mSize, std::vector<block>(PAILLIER_CIPHER_SIZE_IN_BLOCK));
            ...
            //         codeWords_fmat_net[i * PAILLIER_CIPHER_SIZE_IN_BLOCK + j] = codeWords_fmat[i][j];
            //     }
            // }

            //std::cout << "rb_okvs.msize = "<< rb_okvs.mSize << std::endl;

            //std::cout << "fmat_paillier_recv_online: codeWords_fmat_net send begin" << std::endl;
            // coproto::sync_wait((*channel).flush());
            ...
            vec_masked_distance = ipcl::CipherText(paillier_key.pub_key, masked_distance_temp);
            ...
            std::vector<u32> masked_distance;
            fm_paillier::receiver_get_masked_distance_paillier_lp(vec_masked_distance, masked_distance, paillier_key);
            ...
            for(u64 i = 0; i < send_set_size; i++){
                send_prefixes_k[i].insert(send_prefixes_k[i].end(), send_prefixes_k_net.begin() + i * num_prefix_send, send_prefixes_k_net.begin() + i * num_prefix_send + num_prefix_send);
            }
            ////////////////////////////////////////
        ...
        ipcl::PublicKey paillier_pub_key, DH25519_number send_dh_k
            ...
            //     for(u64 j = 0; j < PAILLIER_CIPHER_SIZE_IN_BLOCK; j++){
            //         codeWords_fmat[i][j] = codeWords_fmat_net[i * PAILLIER_CIPHER_SIZE_IN_BLOCK + j];
            //     }
            // }


            ipcl::CipherText vec_masked_distance;
            fm_paillier::sender_q_to_masked_distance_paillier_lp(*sender_elements, *send_vec_dhkk_seedsum, codeWords_fmat, *vec_mask_ct, vec_masked_distance, dimension, fmat_keys_size, paillier_pub_key);
            ...
            std::vector<std::vector<DH25519_point>> recv_prefixes_k(send_set_size), recv_prefixes_kk;
            ////////////////////////////////////////
            //3. receiver sends recv_prefixes_k to receiver.
            ////////////////////////////////////////
            ////////////////////////////////////////
            ...
            // for(u64 i = 0; i < send_set_size; i++){
            //     send_msg[i][1] = block((*sender_elements)[i][0], (*sender_elements)[i][1]);
            // }
        ...
        ipcl::KeyPair paillier_key, const std::string& result_file
            ...
            // std::cout << "[recv]: encode done, send begin" << std::endl;
            // std::cout << "[recv]: send size = " << codeWords_fmat_net.size() << std::endl;
            ...
            // std::cout << "[recv]: recv paillier cipher done" << masked_distance_temp[0] << std::endl;
            ...
            fm_paillier::receiver_get_result_paillier_linfty(vec_masked_distance, masked_distance, paillier_key);
        ...
        ipcl::PublicKey paillier_pub_key
            ...
            fm_paillier::sender_q_to_masked_distance_paillier_linfty(*sender_elements, *send_vec_dhkk_seedsum, codeWords_fmat, vec_masked_distance, additive_masks, dimension, fmat_keys_size, paillier_pub_key);
            ...
            // std::cout << "[send]: additive_masks[0] done" << additive_masks[0] << std::endl;
            // std::cout << "[send]: send paillier cipher done" << vec_masked_distance.getElement(0) << std::endl;

            // auto vec_masked_distance_check = ipcl::CipherText(paillier_pub_key, vec_masked_distance.getElement(0));
