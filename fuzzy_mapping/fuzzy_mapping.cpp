...
namespace osuCrypto
{
    void printBlock1(const osuCrypto::block& b, std::ostream& os = std::cout){
    ...
    std::string point_to_string(Rist25519_point P) {
        std::stringstream ss;
        ss << std::hex << std::setfill('0');
        for (size_t i = 0; i < P.size; ++i) {
            ss << std::setw(2) << static_cast<int>(P.data[i]);
        }
        return ss.str();
    }

std::string monty25519ToString1(const osuCrypto::Sodium::Monty25519& point) {
    std::stringstream ss;
    ss << std::hex << std::setfill('0');
...
// 辅助：打印单个元素（顺序：block -> Monty25519 -> Rist25519_point -> 可流式输出 -> 否则 "print error"）
...
void printElementOrError(const Elem& e, std::ostream& os) {
    ...
    else if constexpr (std::is_same_v<DecayElem, Rist25519_point>) {
        os << point_to_string(e);
    }
...
void debugSend(coproto::Socket* socket, const T& data, const std::string& description = "") {
    ...
    // vector<char>/vector<unsigned char>/vector<uint8_t>
    ...
    // std::array<Rist25519_point, 2> 特判
    else if constexpr (std::is_same_v<DecayT, std::array<Rist25519_point, 2>>) {
        debugFile << "[";
        debugFile << point_to_string(data[0]) << ", " << point_to_string(data[1]);
        debugFile << "]";
    }
    // 其它 std::vector<...>：逐元素打印（元素不可打印则 "print error"）
...
void debugRecvResize(coproto::Socket* socket, T& data, const std::string& description = "") {
    ...
    namespace fmap{
        ...
        void get_interval(const u64& x, const i32& delta, const i32& side_length, segment& interval){
            interval[0] = (u64)((u64)((x - delta) / side_length));
            interval[1] = (u64)((u64)((x + delta) / side_length));
            return;
        }

        void point_to_block_vector(const Rist25519_point& point, const std::vector<block>::iterator& block_vec){
            ...
            if(sizeof(block) * 2 == POINT_LENGTH_IN_BYTE){
                ...
                throw std::runtime_error("no proper parameter for point [lsOPRF.cpp: point_to_block_vector]");
            }
            return;
        }

        void block_vector_to_point(const std::vector<block>::iterator& block_vec, Rist25519_point& point){
            u8 temp_bytes[POINT_LENGTH_IN_BYTE];
        ...
        Rist25519_point get_own_seed(const std::vector<u64>& element, const std::vector<std::vector<segment>>& segments_in_dimensions, const std::vector<std::vector<Rist25519_number>>& values, const u32& dimension, const i32& side_length){
            Rist25519_number sum(values[0][find(segments_in_dimensions[0], element[0] / side_length)]);
            for(auto i = 1; i < dimension; i++){
                sum = sum + values[i][find(segments_in_dimensions[i], element[i] / side_length)];
            }
    ...
    ////////////////////////////////////
        ...
        const u32& dimension, const i32& delta, const i32& side_length, const Rist25519_number& sk){
            ...
            //PRNG prng(osuCrypto::block(0,0));

            Rist25519_number rG, sk_rG;
            u64 num_interval_per_element_per_dimension(((2 * delta + 1) / side_length) + 2);
...
//////////////////////////////////
        ...
        const u32& dimension, const i32& side_length){
            for(auto iter : elements){
                seeds.push_back(get_own_seed(iter, segments_in_dimensions, values, dimension, side_length));
            }
            return;
        }
        ...
        const u32& dimension){
            std::vector<Rist25519_number> temp_Rist25519_number_vector(EC_CIPHER_SIZE_IN_NUMBER);
            for(u64 i = 0; i < dimension; i++){
                for(u64 j = 0; j < segments_in_dimensions[i].size(); j++){
                    for(u64 k = segments_in_dimensions[i][j][0]; k < segments_in_dimensions[i][j][1] + 1; k++){
                        keys.push_back(block(i, k));
                        // point_to_block_vector(vals_candidate_r.top(), temp_block_vector.begin());
                        // point_to_block_vector(vals_candidate_skr.top() + values[i][j], temp_block_vector.begin() + 2);
                        //point_to_block_vector( values[i][j], temp_block_vector.begin() + 2);
                        temp_Rist25519_number_vector[0] = vals_candidate_r.top();
                        temp_Rist25519_number_vector[1] = vals_candidate_skr.top() + values[i][j];
        ...
        const u32& dimension, const i32& side_length, const i32& okvs_n, const std::array<Rist25519_point, 2>& pk){
            ...
            for(auto i = 0; i < elements.size(); i++){
                ...
                for(u64 j = 0; j < dimension; j++){
                    ...
                    auto value = rb_okvs.decode(codeWords, block(j, k), EC_CIPHER_SIZE_IN_NUMBER);
...
///////////////////////////////////////////////////////////////////////
        ...
        const Rist25519_number& sk, const Rist25519_number& dh_sk){
            for(u64 i = 0; i < (vec_enc_mask_seedsum.size() / 2); i++){
                dhk_mask_seedsum.push_back(dh_sk * (vec_enc_mask_seedsum[2 * i + 1] - sk * vec_enc_mask_seedsum[2 * i]));
            }
            // printf("seedsum 369 = \n");
...
///////////////////////////////////////////////////////////////////////
        ...
        Rist25519_number recv_sk, std::array<Rist25519_point, 2> recv_pk, Rist25519_number recv_dh_sk){
            ...
            fmap::elements_to_segments(*receiver_elements, recv_segments_in_dimensions, dimension, delta, side_length);

            std::vector<Rist25519_point> recv_seeds;
            fmap::get_own_seeds(*receiver_elements, recv_segments_in_dimensions, *recv_values, recv_seeds, dimension, side_length);

            std::vector<element> recv_fmap_keys;
            std::vector<std::vector<Rist25519_number>> recv_fmap_vals;
            fmap::get_key_value_pair(recv_segments_in_dimensions, *recv_values, *recv_vals_candidate_r, *recv_vals_candidate_skr, recv_fmap_keys, recv_fmap_vals, dimension);

            RBOKVS_rist recv_fmap_okvs;
            recv_fmap_okvs.init(recv_fmap_keys.size(), 0.1, lambda, seed);
            std::vector<std::vector<Rist25519_point>> recv_fmap_codeWords(recv_fmap_okvs.num_columns, std::vector<Rist25519_point>(EC_CIPHER_SIZE_IN_NUMBER));
            // recv_fmap_okvs.encode(recv_fmap_keys, recv_fmap_vals, EC_CIPHER_SIZE_IN_NUMBER, recv_fmap_codeWords, Rist25519_point::mulGenerator(1));
            recv_fmap_okvs.encode(recv_fmap_keys, recv_fmap_vals, EC_CIPHER_SIZE_IN_NUMBER, recv_fmap_codeWords);


            std::vector<Rist25519_point> recv_fmap_codeWords_net(recv_fmap_okvs.num_columns * EC_CIPHER_SIZE_IN_NUMBER);
            ...
            for(u64 i = 0;i < recv_fmap_okvs.num_columns; i++){
                for(u64 j = 0; j < EC_CIPHER_SIZE_IN_NUMBER; j++){
                    recv_fmap_codeWords_net[i * EC_CIPHER_SIZE_IN_NUMBER + j] = recv_fmap_codeWords[i][j];
                }
            }
            //std::cout << "fmap_recv_online: fmap_codeWords send recv begin" << std::endl;
            // coproto::sync_wait((*channel).send(recv_fmap_codeWords_net));
            debugSend(channel,recv_fmap_codeWords_net,"recv_fmap_codeWords_net");
            //std::cout << "fmap_recv_online: fmap_codeWords send done" << std::endl;
            // coproto::sync_wait((*channel).recvResize(send_fmap_codeWords_net));
            debugRecvResize(channel,send_fmap_codeWords_net,"send_fmap_codeWords_net");
            //std::cout << "fmap_recv_online: fmap_codeWords send recv done" << std::endl;

            
            std::vector<std::vector<Rist25519_point>> send_fmap_codeWords((send_fmap_codeWords_net.size() / EC_CIPHER_SIZE_IN_NUMBER), std::vector<Rist25519_point>(EC_CIPHER_SIZE_IN_NUMBER));
            for(u64 i = 0;i < (send_fmap_codeWords_net.size() / EC_CIPHER_SIZE_IN_NUMBER); i++){
                for(u64 j = 0; j < EC_CIPHER_SIZE_IN_NUMBER; j++){
                    send_fmap_codeWords[i][j] = send_fmap_codeWords_net[i * EC_CIPHER_SIZE_IN_NUMBER + j];
                }
            }

            u64 send_fmap_keys_size;
            //std::cout << "fmap_recv_online: recv_fmap_keys send recv begin" << std::endl;
            // coproto::sync_wait((*channel).send(recv_fmap_keys.size()));
            debugSend(channel,recv_fmap_keys.size(),"recv_fmap_keys.size()");
            //std::cout << "fmap_recv_online: recv_fmap_keys send done" << std::endl;
            // coproto::sync_wait((*channel).recvResize(send_fmap_keys_size));
            debugRecvResize(channel,send_fmap_keys_size,"send_fmap_keys_size");
            //std::cout << "fmap_recv_online: recv_fmap_keys send recv done" << std::endl;
            ...
            debugSend(channel,recv_pk,"recv_pk");
            //std::cout << "fmap_recv_online: recv_pk send done" << std::endl;
            // coproto::sync_wait((*channel).recvResize(send_pk));
            debugRecvResize(channel,send_pk,"send_pk");
            //std::cout << "fmap_recv_online: recv_pk send recv done" << std::endl;

            std::vector<Rist25519_point> recv_vec_enc_mask_seedsum;
            fmap::get_vec_enc_mask_seedsum(*receiver_elements, send_fmap_codeWords, *recv_masks, recv_seeds, recv_vec_enc_mask_seedsum, dimension, side_length, send_fmap_keys_size, send_pk);

            std::vector<Rist25519_point> send_vec_enc_mask_seedsum;
            //std::cout << "fmap_recv_online: vec_enc_mask_seedsum send recv begin" << std::endl;
            // coproto::sync_wait((*channel).send(recv_vec_enc_mask_seedsum));
            debugSend(channel,recv_vec_enc_mask_seedsum,"recv_vec_enc_mask_seedsum");
            //std::cout << "fmap_recv_online: vec_enc_mask_seedsum send done" << std::endl;
            // coproto::sync_wait((*channel).recvResize(send_vec_enc_mask_seedsum));
            debugRecvResize(channel,send_vec_enc_mask_seedsum,"send_vec_enc_mask_seedsum");
            //std::cout << "fmap_recv_online: vec_enc_mask_seedsum send recv done" << std::endl;

            std::vector<Rist25519_point> send_dhk_mask_seedsum;
            fmap::get_dhk_mask_seedsum(send_vec_enc_mask_seedsum, send_dhk_mask_seedsum, recv_sk, recv_dh_sk);

            std::vector<Rist25519_point> recv_dhk_mask_seedsum;
            //std::cout << "fmap_recv_online: dhk_mask_seedsum send recv begin" << std::endl;
            // coproto::sync_wait((*channel).send(send_dhk_mask_seedsum));
            debugSend(channel,send_dhk_mask_seedsum,"send_dhk_mask_seedsum");
            //std::cout << "fmap_recv_online: dhk_mask_seedsum send done" << std::endl;
            // coproto::sync_wait((*channel).recvResize(recv_dhk_mask_seedsum));
            debugRecvResize(channel,recv_dhk_mask_seedsum,"recv_dhk_mask_seedsum");
            //std::cout << "fmap_recv_online: dhk_mask_seedsum send recv done" << std::endl;
            fmap::get_dhkk_seedsum(recv_dhk_mask_seedsum, *recv_masks_inv, *recv_vec_dhkk_seedsum, recv_dh_sk);
        ...
        Rist25519_number send_sk, std::array<Rist25519_point, 2> send_pk, Rist25519_number send_dh_sk){
            ...
            fmap::elements_to_segments(*sender_elements, send_segments_in_dimensions, dimension, delta, side_length);

            std::vector<Rist25519_point> send_seeds;
            fmap::get_own_seeds(*sender_elements, send_segments_in_dimensions, *send_values, send_seeds, dimension, side_length);

            std::vector<element> send_fmap_keys;
            std::vector<std::vector<Rist25519_number>> send_fmap_vals;
            fmap::get_key_value_pair(send_segments_in_dimensions, *send_values, *send_vals_candidate_r, *send_vals_candidate_skr, send_fmap_keys, send_fmap_vals, dimension);

            RBOKVS_rist send_fmap_okvs;
            send_fmap_okvs.init(send_fmap_keys.size(), 0.1, lambda, seed);
            std::vector<std::vector<Rist25519_point>> send_fmap_codeWords(send_fmap_okvs.num_columns, std::vector<Rist25519_point>(EC_CIPHER_SIZE_IN_NUMBER));
            // send_fmap_okvs.encode(send_fmap_keys, send_fmap_vals, EC_CIPHER_SIZE_IN_NUMBER, send_fmap_codeWords, Rist25519_point::mulGenerator(1));
            send_fmap_okvs.encode(send_fmap_keys, send_fmap_vals, EC_CIPHER_SIZE_IN_NUMBER, send_fmap_codeWords);

            std::vector<Rist25519_point> send_fmap_codeWords_net(send_fmap_okvs.num_columns * EC_CIPHER_SIZE_IN_NUMBER);
            std::vector<Rist25519_point> recv_fmap_codeWords_net;

            for(u64 i = 0;i < send_fmap_okvs.num_columns; i++){
                for(u64 j = 0; j < EC_CIPHER_SIZE_IN_NUMBER; j++){
                    send_fmap_codeWords_net[i * EC_CIPHER_SIZE_IN_NUMBER + j] = send_fmap_codeWords[i][j];
                }
            }
            // coproto::sync_wait((*channel).recvResize(recv_fmap_codeWords_net));
            debugRecvResize(channel,recv_fmap_codeWords_net,"recv_fmap_codeWords_net");
            // coproto::sync_wait((*channel).send(send_fmap_codeWords_net));
            debugSend(channel,send_fmap_codeWords_net,"send_fmap_codeWords_net");

            
            std::vector<std::vector<Rist25519_point>> recv_fmap_codeWords((recv_fmap_codeWords_net.size() / EC_CIPHER_SIZE_IN_NUMBER), std::vector<Rist25519_point>(EC_CIPHER_SIZE_IN_NUMBER));
            for(u64 i = 0;i < (recv_fmap_codeWords_net.size() / EC_CIPHER_SIZE_IN_NUMBER); i++){
                for(u64 j = 0; j < EC_CIPHER_SIZE_IN_NUMBER; j++){
                    recv_fmap_codeWords[i][j] = recv_fmap_codeWords_net[i * EC_CIPHER_SIZE_IN_NUMBER + j];
                }
            }
            ...
            debugRecvResize(channel,recv_fmap_keys_size,"recv_fmap_keys_size");
            // coproto::sync_wait((*channel).send(send_fmap_keys.size()));
            debugSend(channel,send_fmap_keys.size(),"send_fmap_keys.size()");

            std::array<Rist25519_point, 2> recv_pk;
            // coproto::sync_wait((*channel).recvResize(recv_pk));
            debugRecvResize(channel,recv_pk,"recv_pk");
            //coproto::sync_wait((*channel).send(send_pk));
            debugSend(channel,send_pk,"send_pk");

            std::vector<Rist25519_point> send_vec_enc_mask_seedsum;
            fmap::get_vec_enc_mask_seedsum(*sender_elements, recv_fmap_codeWords, *send_masks, send_seeds, send_vec_enc_mask_seedsum, dimension, side_length, recv_fmap_keys_size, recv_pk);

            std::vector<Rist25519_point> recv_vec_enc_mask_seedsum;
            // coproto::sync_wait((*channel).recvResize(recv_vec_enc_mask_seedsum));
            debugRecvResize(channel,recv_vec_enc_mask_seedsum,"recv_vec_enc_mask_seedsum");
            //coproto::sync_wait((*channel).send(send_vec_enc_mask_seedsum));
            debugSend(channel,send_vec_enc_mask_seedsum,"send_vec_enc_mask_seedsum");

            std::vector<Rist25519_point> recv_dhk_mask_seedsum;
            fmap::get_dhk_mask_seedsum(recv_vec_enc_mask_seedsum, recv_dhk_mask_seedsum, send_sk, send_dh_sk);

            std::vector<Rist25519_point> send_dhk_mask_seedsum;
            // coproto::sync_wait((*channel).recvResize(send_dhk_mask_seedsum));
            debugRecvResize(channel,send_dhk_mask_seedsum,"send_dhk_mask_seedsum");
            // coproto::sync_wait((*channel).send(recv_dhk_mask_seedsum));
            debugSend(channel,recv_dhk_mask_seedsum,"recv_dhk_mask_seedsum");
            fmap::get_dhkk_seedsum(send_dhk_mask_seedsum, *send_masks_inv, *send_vec_dhkk_seedsum, send_dh_sk);
            //std::this_thread::sleep_for(std::chrono::seconds(1));
            return;
        }
